...
Usage:
    python orchestrate_workflow.py --config-name dataset_cellxgene_pseudo_bulk_3_5k
"""
...
class WorkflowOrchestrator:
    ...
    def validate_config_sync(self, config: DictConfig, force: bool = False) -> None:
        """Validate that the remote config matches the local one."""
        if not force:
            # Get the actual config name from Hydra's config name
            # This should be the full config name like 'dataset_test_workflow'
            config_name = config.get("_config_name_", config.dataset.name)
            if not config_name.startswith("dataset_"):
                config_name = f"dataset_{config_name}"
    ...
    def run_download_step(self, config: DictConfig) -> Optional[int]:
        ...
        # Use the config name as provided when running the workflow script
        # This should be the full config name like 'dataset_test_workflow'
        config_name = config.get("_config_name_", "dataset_test_workflow")

        # Pass the dataset config name as environment variable
        env_vars = {"DATASET_CONFIG": config_name}

        job_id = self._submit_slurm_job(
            self.cpu_login["host"],  # Use hostname from dict
            script_path,
            partition=config.workflow.cpu_partition,  # Use partition from config
            env_vars=env_vars,
        )
        return job_id

    def run_preprocessing_step(
        self, config: DictConfig, dependency_job_id: Optional[int] = None
    ) -> Optional[int]:
    ...
    def run_embedding_step(
        ...
        job_id = self._submit_slurm_job(
            ...
            script_path,
            partition=config.workflow.gpu_partition,  # Use GPU partition
            dependencies=dependencies,
            env_vars=env_vars,
        )
        return job_id
    ...
    def run_dataset_creation_step(
        ...
        job_id = self._submit_slurm_job(
            ...
            script_path,
            partition=config.workflow.cpu_partition,  # Use CPU partition
            dependencies=dependencies,
            env_vars=env_vars,
        )
        return job_id

    def run_workflow(self, config: DictConfig, force: bool = False) -> None:
        """Run the complete workflow."""
        logger.info(f"Starting workflow for dataset: {config.dataset.name}")

        # Validate config synchronization unless forced
        self.validate_config_sync(config, force=force)

        # Step 1: Download (if enabled)
        download_job_id = None
        download_enabled = getattr(config.download, "enabled", True)
        if download_enabled:
            download_job_id = self.run_download_step(config)
            logger.info(
        ...
        preprocessing_enabled = getattr(config.preprocessing, "enabled", True)
        if preprocessing_enabled:
            preprocessing_job_id = self.run_preprocessing_step(
                config, dependency_job_id=download_job_id
            )
            logger.info(
        ...
        embedding_enabled = getattr(config.embedding, "enabled", True)
        if embedding_enabled:
            embedding_job_id = self.run_embedding_step(
                config, dependency_job_id=preprocessing_job_id
            )
            logger.info(
        ...
        dataset_creation_enabled = getattr(config.dataset_creation, "enabled", True)
        if dataset_creation_enabled:
            dataset_job_id = self.run_dataset_creation_step(
                config, dependency_job_id=embedding_job_id
            )
            logger.info(
...
def main(cfg: DictConfig):
    ...
    orchestrator.run_workflow(cfg, force=force)
