...
def create_cell_sentences(
    ...
    """
    Generates three new columns in adata.obs based on highly variable genes
    and a given cell type annotation (annotation_column).

    The three new columns are:
      - "cell_sentence": A space‐separated string of the top cs_length highly variable gene names for that cell.
      - "semantic_true": A semantic sentence generated from the same gene list for that cell.
      - "semantic_similar": A semantic sentence taken from a different cell within the same cell type.
    ...
    log_transform : bool, optional
        If True, the expression values are log-transformed (using np.log1p) before ranking.
        Defaults to True.
    expression_layer : str | None, optional
        If provided, use adata.layers[expression_layer] as the expression data. Otherwise,
        adata.X is used.

    Returns
    -------
    anndata.AnnData
        The AnnData object with three new columns in adata.obs:
          "cell_sentence", "semantic_true", and "semantic_similar".

    Raises
    ------
    ValueError
        If annotation_column is not found in adata.obs.
    """
    # Verify that the annotation column is present.
